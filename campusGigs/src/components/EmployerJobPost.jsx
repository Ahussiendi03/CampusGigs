...
const EmployerJobPost = () => {
  ...
  useEffect(() => {
    const fetchEmployerData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/jobPosts/employers/${employerId}`
        );
    ...
    const fetchSavedPositions = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/employer/${employerId}/positions`
        );
  ...
  const handleDismissSubmit = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/jobposts/dismiss/${selectedJobId}`,
        {
      ...
      <div className="flex-1 flex flex-col items-center mb-48 p-4">
        <div className="mt-6 w-full">
          <div className="w-full flex justify-between items-center mb-3">
            ...
            <button
              onClick={
                employerData.status === "approved" ? togglePostModal : undefined
              }
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm text-gray-600">
                      {jobData.salaryRateType === "per day"
                        ? "₱/day"
                        : "₱/hour"}
                    </span>
                  </div>
